...
def group_sort(net, num_groups=4):
    ...
    groups_sorted = []
    for g in groups:
        groups_sorted.append(randomize_channels(g))
    return tf.concat(groups_sorted, axis=3)


def randomize_channels(net):
    net_shape = net.get_shape().as_list()
    print('net_shape: {}'.format(net_shape))
    _, ch_idx = tf.nn.top_k(tf.random_uniform((net_shape[0], 1, 1, net_shape[3])), k=net_shape[3])
