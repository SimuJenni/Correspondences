    ...
    def train(self, chpt_path=None):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                # Create the model
                center_pred = self.model.net(tiles_train, reuse=None, train=True)

                # Compute losses
                total_loss = self.model.classification_loss()
                ...
                tf.summary.image('weights/chan', weights_montage(weights_disc_1, 6, 16),
                                 max_outputs=1)

                # Generator training operations
                train_op = self.make_train_op(total_loss, self.optimizer())
