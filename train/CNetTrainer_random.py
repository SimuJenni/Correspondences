...
class CNetTrainer:
    ...
    def get_train_batch(self):
        with tf.device('/cpu:0'):
            ...
            # Pre-process data
            tiles_train = self.pre_processor.process_train(img_train)
    ...
    def train(self, chpt_path=None):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                tiles_train = self.get_train_batch()

                # Create the model
                _ = self.model.net(tiles_train, reuse=None, train=True)

                # Compute losses
                order_loss = self.model.classification_loss()
                pos_loss = self.model.position_loss()

                # Handle dependencies with update_ops (batch-norm)
                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                if update_ops:
                    updates = tf.group(*update_ops)
                    order_loss = control_flow_ops.with_dependencies([updates], order_loss)
                    pos_loss = control_flow_ops.with_dependencies([updates], pos_loss)

                # Make summaries
                self.make_summaries()
                with tf.variable_scope('encoder', reuse=True):
                    weights_disc = slim.variable('conv_1/weights')
                tf.summary.image('weights/conv_1', weights_montage(weights_disc, 6, 16),
                                 max_outputs=1)

                # Generator training operations
                train_op1 = self.make_train_op(order_loss, self.optimizer(), scope='order_classifier,encoder')
    ...
    def transfer_finetune(self, chpt_path, num_conv2train=None, num_conv2init=None, dataset_id=None):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                weights = tf.unstack(weights_disc_1, 2, axis=3)
                tf.summary.image('weights/chan_1', weights_montage(weights[0], 6, 16),
                                 max_outputs=1)
                tf.summary.image('weights/chan_2', weights_montage(weights[1], 6, 16),
                                 max_outputs=1)
